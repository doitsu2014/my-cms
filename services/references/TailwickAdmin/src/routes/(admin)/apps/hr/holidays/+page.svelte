<script>
    ...
    import holidaysList from "../../../../../common/data/holidays"
    ...
    <div class="card">
        <div class="card-body">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div class="shrink-0">
                    <a href="#!" type="button" class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20 add-employee" on:click={toggleAddModal}><LucideIcon name="Plus" class="inline-block size-4"/> <span class="align-middle">Add Holiday</span></a>
                </div>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="w-full whitespace-nowrap">
                    ...
                    <tbody class="list" id="employeeList">
                        {#each holidaysList as holiday}
                        <tr>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{holiday.id}</td>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{holiday.day}</td>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{holiday.date}</td>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{holiday.holidayname}</td>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">{holiday.type}</td>
                            <td class="px-3.5 py-2.5 border border-slate-200 dark:border-zink-500">
                                <div class="flex gap-2">
                                    <a href="#!" class="flex items-center justify-center size-8 transition-all duration-200 ease-linear rounded-md bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500" on:click={toggleAddModal}><LucideIcon name="Pencil" class="size-4"/></a>
                                    <a href="#!" class="flex items-center justify-center size-8 transition-all duration-200 ease-linear rounded-md bg-slate-100 text-slate-500 hover:text-red-500 hover:bg-red-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:text-red-500 dark:hover:bg-red-500/20" on:click={toggleDelete}><LucideIcon name="Trash2" class="size-4"/></a>
                                </div>
                            </td>
                        </tr>  
                        {/each}
...
<Modal modal-center className="-translate-y-2/4" isOpen={isAddModal} toggle={toggleAddModal}>
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
            ...
            <button class="transition-all duration-200 ease-linear text-slate-400 hover:text-red-500" on:click={toggleAddModal}><LucideIcon name="X" class="size-5"/></button>
        </div>
