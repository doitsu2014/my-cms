...
impl Post {
    ...
    pub fn find_all(conn: &mut PgConnection) -> QueryResult<Vec<Post>> {
        crate::schema::posts::table.load(conn)
    }
}

#[cfg(test)]
mod tests {
    use crate::{domains::post::Post, infrastructure};
    ...
    fn test_post_save() {
        ...
        infrastructure::database::migrate_database(&connection_string);
        let mut conn = infrastructure::database::establish_connection(&connection_string);
        ...
        let posts = Post::find_all(&mut conn).unwrap();
