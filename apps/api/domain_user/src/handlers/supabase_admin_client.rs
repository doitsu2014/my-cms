//! `SupabaseAdminClient` — Supabase GoTrue admin API adapter used by the
//! user domain. Implements the create / read / update / delete and
//! password-reset operations against the GoTrue admin endpoints.
...
use crate::{
    ...
    dto::{is_recognised_role, AppUserModel, BAN_DURATION},
    handlers::{
        create::create_request::CreateUserRequest, modify::modify_request::ModifyUserRequest,
    },
};
        ...
        match req.banned {
            Some(true) => {
                body.insert(
                    ...
                    Value::String(BAN_DURATION.to_string()),
                );
            }
            Some(false) => {
                body.insert(
