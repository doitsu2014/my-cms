...
//! user domain.
//!
//! Mirrors `domain_posts::domain::error::AppError`. The legacy
//! `application_core::common::app_error::AppError` is now a re-export
//! shim that forwards to this type so that other consumers (e.g.
//! `presentation_models::api_response`) keep compiling during the
//! transition.
