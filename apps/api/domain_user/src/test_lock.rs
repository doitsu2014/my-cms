//! Test-only helpers shared across `domain_user` modules.
//!
...
//!
//! Mirrors `apps/api/domain_auth/src/test_lock.rs` so the pattern is
...
/// manipulation tests across `domain_user`.
