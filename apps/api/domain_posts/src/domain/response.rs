...
//!
//! Moved from `apps/api/src/presentation_models/api_response.rs` per design
//! Decision 2. The legacy `cms::presentation_models::api_response` is now a
//! re-export of this module so existing call sites keep working during the
//! transition.
...
mod tests {
    ...
    fn test_case_conflict_from_app_error() {
        let app_error = AppError::Conflict("duplicate email".to_string());
