...
- Record bounded operational attributes, including the explicitly approved raw client IP and User-Agent plus derived browser, operating-system, and device fields, while continuing to exclude authentication and GraphQL payload data.
- Preserve the original visitor IP through the Cloudflare-to-Traefik boundary by trusting `X-Forwarded-*` only from Cloudflare's published CIDRs and preventing public bypass of Traefik.
...
- Does not change API behavior, collector or Jaeger deployment, public website functionality, browser telemetry, analytics, or health-check semantics. Raw client identifiers are restricted operational telemetry and require access/retention controls in Jaeger.
