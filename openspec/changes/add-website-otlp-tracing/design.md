...
- Keep telemetry opt-in, fail-open, bounded, privacy-safe, and invisible to browser runtime configuration.
...
### 3. Treat trace data as restricted operational metadata

**Driver:** The public reader's URL, inbound headers, SSR content, and GraphQL requests may contain user or content data unsuitable for Jaeger.

**Decision:** Explicit instrumentation writes an allow-list only. It must never set `url.full`, query-string values, request/response body values, rendered HTML, GraphQL document/operation text, GraphQL variables, cookies, `Authorization`, other auth headers, exporter headers, or environment-variable values. Error handling records a stable error category/status and response status only; it must not attach a caught error message, stack, or upstream response body. The exporter must receive no custom headers unless a separately reviewed credential contract is introduced.
...
**Consequences:** Jaeger can answer which route called which backend endpoint, duration, status, and trace relationship, but cannot reproduce user content or GraphQL input. If richer diagnostics are needed later, they require a new data-classification design.
...
- **Span attributes leak content or credentials** → use explicit/manual allow-listed spans instead of broad auto instrumentation; inspect finished in-memory spans for forbidden values.
