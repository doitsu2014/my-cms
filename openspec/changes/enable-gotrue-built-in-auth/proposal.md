...
The frontend currently calls `supabase.auth.signInWithOAuth({ provider: "keycloak" })` (see `apps/web/src/auth/AuthContext.tsx:75-78`), but the local Supabase GoTrue instance does not have Keycloak configured as an external OAuth provider. The result is `{"code":400,"error_code":"validation_failed","msg":"Unsupported provider: provider is not enabled"}` returned by GoTrue on every login attempt, making the admin UI unreachable. The product does not need (and does not want to operate) a Keycloak server. The fix is to use GoTrue's built-in email+password sign-in path, pre-seeding a single administrator account out-of-band so public sign-up can stay closed.

## What Changes

- **Add a real login page** at `apps/web/src/app/admin/login/page.tsx` that renders an email + password form and calls `supabase.auth.signInWithPassword`. Replaces the implicit "you'll be redirected somewhere" flow.
...
- **Drop the dead `_keycloak?: unknown` field** from `apps/web/src/config/api.config.ts` (the only remaining `keycloak` mention in the frontend source).

The backend `SupabaseAuthLayer` (`apps/api/src/common/supabase_auth.rs`) is **not modified**. The JWT shape GoTrue issues from `signInWithPassword` is identical to what it would have issued from an OAuth flow: same `sub`, `email`, `role`, `app_metadata` claims, same HS256 signing with the same `JWT_SECRET`, same `aud: "authenticated"`. The existing role-enforcement check (`required_roles` reading `app_metadata.roles`) keeps working unchanged.
...
- **New frontend files**: `apps/web/src/app/admin/login/page.tsx` (login form), `apps/web/src/app/admin/login/schema.ts` (Zod validation), `apps/web/src/app/admin/login/schema.test.ts` (unit tests), `apps/web/src/app/admin/login/page.test.tsx` (component tests), `apps/web/vitest.config.ts` (test runner config), `apps/web/src/test/setup.ts` (jest-dom matchers loader).
- **Modified frontend files**: `apps/web/src/auth/AuthContext.tsx` (replace `login` body, drop `signInWithOAuth` import), `apps/web/src/auth/ProtectedRoute.tsx` (drop callback detection + sessionStorage + auto-trigger), `apps/web/src/app/admin/login/page.tsx` (add `noValidate` to the form so Zod is the sole validation source), `apps/web/src/app/admin/components/top-bar.tsx` (no change — already compatible), `apps/web/src/config/api.config.ts` (drop `_keycloak` field), `apps/web/src/App.tsx` (register the new `/admin/login` route), `apps/web/package.json` (add `test` and `test:watch` scripts + vitest / testing-library devDeps), `apps/web/tsconfig.json` (add vitest globals + jest-dom types).
