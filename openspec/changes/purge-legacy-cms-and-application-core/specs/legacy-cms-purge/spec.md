...
## Purpose
Define the Phase A workspace cleanup that retires the legacy `cms` root package, the `legacy_bootstrap` binary, the `application_core` transitional crate, and the stale `application_core`/`migration` path-dependencies on the `gateway` crate, while keeping the `migration` CLI binary and the `migration` library reachable so that `test_helpers` and operator migration workflows continue to compile and run.
